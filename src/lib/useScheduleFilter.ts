...
import type { Schedule, Entry, WeeklyScheduleEntry } from '../components/schedule-form'

// Custom hook for schedule filtering logic
export function useScheduleFilter(schedules: Schedule[], selectedAssignee: string) {
  ...
  const filteredEntriesBySchedule = useMemo(() => {
    const result: { [scheduleId: string]: (Entry | WeeklyScheduleEntry)[] } = {}

    schedules.forEach((schedule) => {
      const scheduleEntries: (Entry | WeeklyScheduleEntry)[] = []
      ...
      if (
        ...
        // Weekly format: filter weekly entries by assignee
        schedule.weeklyEntries.forEach((entry) => {
        ...
        // Date-specific format: filter entries by assignee
        schedule.entries.forEach((entry) => {
          if (selectedAssignee === 'all' || entry.assignee === selectedAssignee) {
            scheduleEntries.push(entry)
          }
        ...
        try {
          ...
          console.warn(
            `🔍 useScheduleFilter - skipping invalid legacy schedule ${schedule.id}:`,
            error
          )
  ...
  const filteredSchedules = useMemo(() => {
    return schedules.filter((schedule) => {
      return (
        filteredEntriesBySchedule[schedule.id] && filteredEntriesBySchedule[schedule.id].length > 0
      )
    })
